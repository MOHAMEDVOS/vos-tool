...
def show_pending_phrases_tab(learning_manager):
    ...
    # Group by quality tier (using quality_score if available, fallback to confidence)
    auto_approve = [p for p in pending_phrases if p.get("quality_tier") == "auto_approve" or (p.get("quality_score", 0) >= 0.90)]
    high_priority = [p for p in pending_phrases if p.get("quality_tier") == "high_value" or (0.80 <= p.get("quality_score", p.get("confidence", 0)) < 0.90)]
    medium_priority = [p for p in pending_phrases if p.get("quality_tier") == "medium_value" or (0.65 <= p.get("quality_score", p.get("confidence", 0)) < 0.80)]
    low_priority = [p for p in pending_phrases if p.get("quality_tier") == "low_value" or (p.get("quality_score", p.get("confidence", 0)) < 0.65)]
    
    # Action buttons for batch operations
    if pending_phrases:
        # Add custom styling for action buttons to match app theme
        st.markdown("""
        <style>
        /* Style action buttons in Phrases Pending Review section */
        .stButton > button,
        button[kind="primary"],
        ...
        button[kind="primary"],
        div[data-testid="column"] button[kind="primary"] {
            background: linear-gradient(135deg, rgba(37, 99, 235, 0.2) 0%, rgba(37, 99, 235, 0.15) 100%) !important;
        ...
        with action_col1:
            # Use quality score if available, fallback to confidence
            auto_approve_count = len(auto_approve) if auto_approve else 0
            high_priority_count = len(high_priority)
            total_high_quality = auto_approve_count + high_priority_count
            
            if st.button(f"Approve All High Quality (≥90%)", type="primary", 
                        help=f"Approve all {total_high_quality} high quality phrases (Quality Score ≥ 90%)"):
                with st.spinner("Approving high quality phrases..."):
                    # Try quality score first, fallback to confidence
                    result = learning_manager.approve_by_quality_score(min_quality_score=0.90)
                    if not result.get('success') or result.get('stats', {}).get('total_approved', 0) == 0:
                        # Fallback to confidence-based approval
                        result = learning_manager.approve_all_high_priority_phrases(min_confidence=0.90)
                    
                    if result.get('success'):
                        st.success(result.get('message', 'Approved successfully'))
                        st.rerun()
                    else:
                        st.error(result.get('message', 'Failed to approve phrases'))
        
        with action_col2:
            total_pending = len(pending_phrases)
            if st.button(f"Approve All Pending ({total_pending})", 
                        type="secondary",
                        help=f"Approve all {total_pending} pending phrases regardless of quality score"):
                with st.spinner(f"Approving all {total_pending} pending phrases..."):
                    result = learning_manager.approve_all_pending_phrases()
                    if result.get('success'):
                        st.success(result.get('message', 'All phrases approved successfully'))
                        st.rerun()
                    else:
                        st.error(result.get('message', 'Failed to approve phrases'))
        
        with action_col3:
            # Check for duplicates (same phrase text ONLY, case-insensitive) - not category
            phrase_groups = {}
            duplicates_count = 0
            for p in pending_phrases:
                normalized = p['phrase'].lower().strip() if p.get('phrase') else ""
                # Use phrase text only as key (not category) - matches backend logic
                key = normalized
                if normalized:
            ...
            # Count total duplicate entries (excluding the first one in each group)
            duplicates_count = sum(len(phrases) - 1 for phrases in phrase_groups.values() if len(phrases) > 1)
            
            if st.button(f"Remove Duplicates ({duplicates_count} found)", 
                        type="secondary",
                        help=f"Remove {duplicates_count} duplicate phrases, keeping the best version"):
                with st.spinner("Removing duplicates..."):
                    result = learning_manager.remove_duplicate_phrases()
                    if result.get('success'):
                        st.success(result.get('message', 'Duplicates removed successfully'))
                        st.rerun()
                    else:
                        st.error(result.get('message', 'Failed to remove duplicates'))
        
        with action_col4:
            total_high_quality = len(auto_approve) + len(high_priority) if auto_approve else len(high_priority)
            st.metric("High Quality", total_high_quality)
        
        st.markdown("---")

    if auto_approve:
        st.markdown("### 🟢 Auto-Approve (Quality Score ≥ 90%)")
        for phrase in auto_approve:
            show_phrase_card(phrase, learning_manager, priority="auto_approve")
    
    if high_priority:
        st.markdown("### 🟡 High Priority (Quality Score 80-89%)")
            ...
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
                <div>
                    ...
                    <p style="color: #94a3b8; margin: 0; font-size: 0.9rem;">
                        ...
                        Quality Score: {phrase.get('quality_score', phrase['confidence']):.1%} |
