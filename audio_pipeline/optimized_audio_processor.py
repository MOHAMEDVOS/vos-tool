"""
Optimized audio processor for heavy audits - AssemblyAI only + basic detections.
No local semantic models, no Egyptian correction - just fast AssemblyAI + local checks.
"""
...
class OptimizedAudioProcessor:
    """Optimized audio processor using AssemblyAI only - no local models."""
    ...
    def process_single_file_optimized(
        self, 
        file_path: Path, 
        ...
        """
        Optimized single-file processing - AssemblyAI + basic detections only.
        No semantic models, no Egyptian correction - FAST!
        
        Args:
            file_path: Path to audio file
            additional_metadata: Additional metadata to include
            include_debug: Include debug information (ignored for speed)
        ...
        logger.info(f"Starting OPTIMIZED processing of file: {file_path}")
        ...
        # FAST parallel processing: AssemblyAI + basic detections only
        try:
            result = self._process_optimized_parallel(
            ...
            logger.error(f"Optimized processing failed for {file_path}: {e}", exc_info=True)
            return {
                'agent_name': agent_name,
                'phone_number': phone_number,
                'timestamp': timestamp,
    ...
    def _process_optimized_parallel(
        self, 
        ...
        """Process with optimized parallel execution - AssemblyAI only."""
        ...
        if temp_file:
            ...
            # AssemblyAI transcription (main processing)
            transcript = ""
            transcription_status = "failed"
            transcription_error = None
            ...
            if future_transcription:
                try:
                    transcription_result = future_transcription.result(timeout=180)  # 3 minute timeout
                    ...
                    logger.info(f"AssemblyAI transcription completed: {len(transcript)} chars, status: {transcription_status}")
            ...
            # FAST rebuttal detection - simple keyword matching only
            result['rebuttal_detection'] = self._fast_rebuttal_detection(transcript)
    ...
    def _fast_rebuttal_detection(self, transcript: str) -> Dict[str, Any]:
        """Fast rebuttal detection using simple keyword matching only."""
        if not transcript:
            return {'result': 'No', 'transcript': '', 'method': 'fast_keyword'}
        
        # Simple rebuttal indicators (no semantic analysis)
        rebuttal_indicators = [
