...
def show_campaign_audit_dashboard(dashboard_manager, generate_csv_data):
    ...
    if "campaign_dashboard_data" in st.session_state:
        ...
        csv_data, filename = generate_csv_data(
            df, f"campaign_audit_{selected_campaign}"
        )
        st.download_button(
            label="Download CSV",
            data=csv_data,
...
def show_lite_audit_dashboard(dashboard_manager, generate_csv_data):
    ...
    # Remove non-essential context columns from the Lite Audit dashboard table/CSV,
    # while keeping them in the underlying stored data.
    columns_to_remove = [
        "Rebuttal Detection",
        "Transcription",
        "Agent Intro",
        ...
        "Intro Score",
        "Status",
        # Internal code column; keep user-facing "Audit Type" only in the table
        "audit_type",
    ]
    display_df = df.drop(
    ...
    # Ensure key context columns are visible and ordered in the table
    preferred_columns = [
        "Agent Name",
        "Phone Number",
        "Timestamp",
    ...
    existing_pref = [c for c in preferred_columns if c in display_df.columns]
    remaining_cols = [c for c in display_df.columns if c not in existing_pref]
    if existing_pref:
        display_df = display_df[existing_pref + remaining_cols]
