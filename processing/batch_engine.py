...
class BatchProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        # Optimize workers based on CPU count and GPU availability
        import multiprocessing
        import torch
        cpu_count = multiprocessing.cpu_count()
        
        # GPU-optimized: Use more workers since GPU handles heavy lifting
        if torch.cuda.is_available():
            # With GPU, we can use more CPU workers for I/O and preprocessing
            default_workers = min(cpu_count, 6)  # Use up to 6 workers with GPU
        else:
            # CPU-only: Conservative worker count
            default_workers = min(cpu_count, 4)  # Keep 4 threads for CPU
